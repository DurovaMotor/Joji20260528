(function () {
  var CART_KEY = "hightac-joji20260528-cart-v1";
  var SUBMISSION_KEY = "hightac-joji20260528-last-submission-v1";
  var LANG_KEY = "hightac-joji20260528-language-v1";
  var DEFAULT_LANG = "fr";
  ...
  function getCatalog() {
    ...
    return {
      model: {
        name: "Joji",
        motorPicture: "Database/MotorPictures/Joji.png"
      },
      parts: []
    };
  }
  ...
  function buildPartIndexes() {
    ...
    (state.catalog.parts || []).forEach(function (part) {
      ...
      var newCode = cleanText(part && part.newCode);
      if (code) {
        state.index[code] = part;
      }
      if (newCode) {
        state.index[newCode] = part;
      }
      if (originalCode) {
  ...
  function buildCartItem(part, quantity) {
    return {
      code: getPartDisplayCode(part) || part.code,
      originalCode: part.originalCode || "",
      newCode: part.newCode || "",
  ...
  function getPartMessageName(part) {
    return getPreferredPartName(part, LOCALES.en.unnamed_part);
  }

  function getPartDisplayCode(part) {
    return cleanText(part && part.newCode) ||
      cleanText(part && part.code) ||
      cleanText(part && part.originalCode);
  }
  ...
  function buildPlaceholderImage(part) {
    var code = escapeHtml(getPartDisplayCode(part) || "NO CODE");
  ...
  function renderPartRow(part) {
    ...
    var displayCode = getPartDisplayCode(part);
    ...
    return '' +
      '<article class="part-row" data-part-code="' + escapeHtml(part.code) + '">' +
        ...
        '<div class="part-row__body">' +
          ...
          '<div class="part-row__code">' + t("code_label") + ': ' + escapeHtml(displayCode) + '</div>' +
  ...
  function renderCartDrawer() {
    ...
    itemsHost.innerHTML = summary.items.map(function (item) {
      var displayCode = getPartDisplayCode(item);
      return '' +
        '<article class="cart-item">' +
          ...
          '<div class="cart-item__body">' +
            ...
            '<div class="cart-item__code">' + t("code_label") + ': ' + escapeHtml(displayCode) + '</div>' +
  ...
  function buildWhatsAppMessage(items, modelName) {
    var lines = items.map(function (item, index) {
      ...
      return (index + 1) + ". " + getPartMessageName(item) + " | " + getPartDisplayCode(item) + " | Qty: " + item.quantity + " | Prices: " + formatLatestPriceForMessage(item) + " | Subtotal: " + subtotal;
    });
  ...
  function submitCart() {
    ...
    var modelName = state.catalog.model.name || "Joji";
      ...
      return '' +
        '<article class="summary-item">' +
          '<div class="summary-item__main">' +
            ...
            '<div class="summary-item__code">' + t("code_label") + ': ' + escapeHtml(displayCode) + '</div>' +
